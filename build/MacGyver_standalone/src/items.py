...
class ObjectItems():
    ...
    def getFreeCases(self):
        ...
        self.freeCases = []  # will contain all free cases of the mapp
        for index_x, x in enumerate(self.mapp.grid):
            for index_y, y in enumerate(x):
                if y == " ":

                    self.freeCases.append([index_x, index_y])
        self.new = random.choice(self.freeCases)  # random selected free pos
        return self.new
    ...
    def setObjectsPositions(self):
        ...
        for i in items:
            newPos = self.getFreeCases()  # used method to attribute position
            Nx = newPos[0]
            Ny = newPos[1]
